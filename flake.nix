    ...
    inputs.flake-parts.lib.mkFlake { inherit inputs; } {
      ...
      in {
        devShells.default = pkgs.mkShell {
          ...
          #   PODMAN_FLAKE_CONTAINERS_CONF: The folder under which all generated configs and default storage live.
          #                                 Paths relative to calling dir. Must only be shared among flakes with the
          #                                 same PODMAN_FLAKE_STORAGE_DRIVER, and of roughly similar podman version.
          #                                 Default=.podman-flake
          #   PODMAN_FLAKE_STORAGE_DRIVER: The podman driver type to use. Defaults to match current
          #                                environment's, or falls back to 'overlay'
          #   PODMAN_FLAKE_NETNS_ISOLATE: If non-blank, isolate the rootless network namespace from the host.
          #                               If you do not set this, the podman named networks can conflict with
          #                               all other non-isolated podman named networks on the system.
          #                               Even if you set this, any expliict subnets still need to be globally
          #                               unique since the routing shares the kernel network stack.
          shellHook = ''
            ########################
